...
func invoke() -> void:
	...
	TTUtils.invoke_js_argv(_get_javascript_object(), 'checkScene', args)
	if !is_done: await complete

class SuccessRes0 extends TTObject:
	var is_exist: bool:
		get:
			return _get_by_key_with_default('isExist', false)
