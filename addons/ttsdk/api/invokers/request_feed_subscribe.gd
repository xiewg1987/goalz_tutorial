extends TTAsyncTask
class_name TTAsyncRequestFeedSubscribe
...
var scene: Variant:
	set(value):
		_args_object_set(0, 'scene', value)

var content_ids: Array:
	get:
		if content_ids == null:
			content_ids = []
			_args_object_set(0, 'contentIDs', content_ids)
		return content_ids
	set(value):
		content_ids = value
		_args_object_set(0, 'contentIDs', content_ids)
...
var all_scene: bool:
	set(value):
		_args_object_set(0, 'allScene', value)

var type: String:
	set(value):
		_args_object_set(0, 'type', value)
...
func invoke() -> void:
	...
	TTUtils.invoke_js_argv(_get_javascript_object(), 'requestFeedSubscribe', args)
...
class SuccessRes0 extends TTObject:
	...
	var success: bool:
		get:
			return _get_by_key_with_default('success', false)
