...
var scope: String:
	set(value):
		_args_object_set(0, 'scope', value)
...
func invoke() -> void:
	...
	TTUtils.invoke_js_argv(_get_javascript_object(), 'authorize', args)
...
class SuccessRes0 extends TTObject:
	var data: Dictionary:
		get:
			...
			if jso != null:
				data = TTUtils.dictionary_from_js(jso)
			return data
