...
var force: bool:
	set(value):
		_args_object_set(0, 'force', value)
...
func invoke() -> void:
	...
	TTUtils.invoke_js_argv(_get_javascript_object(), 'login', args)
	if !is_done: await complete

class SuccessRes0 extends TTObject:
	var anonymous_code: String:
		get:
			return _get_by_key_with_default('anonymousCode', '')
	
	var is_login: bool:
		get:
			return _get_by_key_with_default('isLogin', false)
	
	var code: String:
		get:
			return _get_by_key_with_default('code', '')
