extends TTAsyncTask
class_name TTAsyncGetSystemInfo
signal success(res: TT.SystemInfo)

signal fail(res: FailRes0)

signal complete(res: CompleteRes0)

func get_result_success() -> TT.SystemInfo:
...
func invoke() -> void:
	_register_to_owner()
	_args_object_set(0, 'success', _create_signal_callback('success', 1, [1,TT.SystemInfo]))
	...
	TTUtils.invoke_js_argv(_get_javascript_object(), 'getSystemInfo', args)
