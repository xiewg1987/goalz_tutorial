extends TTAsyncTask
class_name TTAsyncRequestSubscribeMessage
...
var tmpl_ids: Array:
	get:
		if tmpl_ids == null:
			tmpl_ids = []
			_args_object_set(0, 'tmplIds', tmpl_ids)
		return tmpl_ids
	set(value):
		tmpl_ids = value
		_args_object_set(0, 'tmplIds', tmpl_ids)
...
func invoke() -> void:
	...
	TTUtils.invoke_js_argv(_get_javascript_object(), 'requestSubscribeMessage', args)
	if !is_done: await complete

class SuccessRes0 extends TTObject:
	var template_id: String:
		get:
			return _get_by_key_with_default('TEMPLATE_ID', '')
