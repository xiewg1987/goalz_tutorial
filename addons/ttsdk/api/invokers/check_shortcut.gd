extends TTAsyncTask
class_name TTAsyncCheckShortcut
...
func invoke() -> void:
	...
	TTUtils.invoke_js_argv(_get_javascript_object(), 'checkShortcut', args)
...
class SuccessRes0 extends TTObject:
	var status: Status:
		get:
			if status == null:
				var jso = _get_by_key('status')
				if jso != null:
					status = Status.new(jso)
			return status
	...
	class Status extends TTObject:
		var exist: bool:
			get:
				return _get_by_key_with_default('exist', false)
		
		var need_update: bool:
			get:
				return _get_by_key_with_default('needUpdate', false)
		
		func _to_string() -> String:
			return 'Status{ ' \
			  + 'exist=' + str(exist) + ', ' \
			  + 'need_update=' + str(need_update) + ', ' \
			+ '}'
