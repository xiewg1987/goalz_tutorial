...
func invoke() -> void:
	...
	TTUtils.invoke_js_argv(_get_javascript_object(), 'openSetting', args)
	if !is_done: await complete

class SuccessRes0 extends TTObject:
	var auth_setting: Dictionary:
		get:
			var jso = _get_by_key('authSetting')
			if jso != null:
				auth_setting = TTUtils.dictionary_from_js(jso)
			return auth_setting
